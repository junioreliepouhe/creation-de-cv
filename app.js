...
/* safeHtml: DOMPurify for enterprise-grade XSS protection */
function safeHtml(s) {
  if (!s) return '';
  if (typeof DOMPurify !== 'undefined') {
    var clean = DOMPurify.sanitize(s, { ALLOWED_TAGS: ['b', 'strong', 'em', 'i', 'br'] });
    return clean.replace(/\n/g, '<br>');
  }
  /* Fallback in case DOMPurify fails to load entirely */
  var escaped = String(s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  escaped = escaped.replace(/&lt;(\/?)strong&gt;/gi, '<$1strong>').replace(/&lt;(\/?)b&gt;/gi, '<$1b>').replace(/&lt;(\/?)em&gt;/gi, '<$1em>').replace(/&lt;(\/?)i&gt;/gi, '<$1i>').replace(/&lt;br\s*\/?&gt;/gi, '<br>');
